...
-- Chỉ còn Game / Player / ESP / Info / Settings
local Tabs = {
...
local GameSection     = Tabs.Game:Section({ Title = "Evade (EVENT) - ZMatrix", Opened = true })
...
-- Avoid Nextbot (đã chuyển xuống GameSection)
GameSection:Toggle({
    Title = "Avoid Nextbot",
    ...
    Callback = function(Value)
        ...
        task.spawn(function()
            while ActiveAutoFarmMoney do
                ...
                -- lấy model mình trong workspace.Game.Players
                ...
                -- nếu không có HRP hoặc mình đang downed hoặc đang revive thì bỏ vòng
                if not hrp or imDown or InReviving then
                    -- chỉ skip lần này, không dùng continue để khỏi lỗi bản Lua cũ
                else
        ...
        task.spawn(function()
            while ActiveEspPlayers do
                ...
                if not gameFolder then
                    -- không có gameFolder thì skip vòng này
                else
