...
local allfunc = {}
local IndexUIColor = {
	["Border Color"] = Color3.fromRGB(142, 212, 21),
	...
	["Search Icon Highlight Color"] = Color3.fromRGB(142, 212, 21),
	...
	["Title Text Color"] = Color3.fromRGB(142, 212, 21),
	["Background Main Color"] = Color3.fromRGB(15, 15, 15),
	["Background 1 Color"] = Color3.fromRGB(28, 28, 34),
	["Background 1 Transparency"] = 0.25,
	["Background 2 Color"] = Color3.fromRGB(90, 90, 90),
	["Background 3 Color"] = Color3.fromRGB(20, 20, 20),
	["Background Image"] = "",
	["Page Selected Color"] = Color3.fromRGB(142, 212, 21), -- Xanh lá trung bình
	["Section Text Color"] = Color3.fromRGB(142, 212, 21),
	["Section Underline Color"] = Color3.fromRGB(142, 212, 21),
	["Toggle Border Color"] = Color3.fromRGB(142, 212, 21),
	...
	["Dropdown Selected Color"] = Color3.fromRGB(80, 180, 80),
	["Dropdown Selected Check Color"] = Color3.fromRGB(142, 212, 21), -- Xanh lá sáng cho check
	["Textbox Highlight Color"] = Color3.fromRGB(142, 212, 21),
	["Box Highlight Color"] = Color3.fromRGB(142, 212, 21),
	["Slider Line Color"] = Color3.fromRGB(90, 138, 12), -- Xanh lá đậm cho nền slider
	["Slider Highlight Color"] = Color3.fromRGB(142, 212, 21), -- Xanh lá sáng cho slider fill
...
cache.UIColor = IndexUIColor
cache.UIToggled = true
local currcolor = {}
local Library = {};
...
Library_Function.Gui.Name = 'noguchihyuga'
Library_Function.Gui.Enabled = false

cache.ReadyForGuiLoaded = false
spawn(function()
	repeat task.wait()
	until cache.ReadyForGuiLoaded
	if cache.UIToggled then
...
Library_Function.Gui.Parent = game.Players.LocalPlayer.PlayerGui
Library_Function.NotiGui.Parent = game.Players.LocalPlayer.PlayerGui
Library_Function.HideGui.Parent = game.Players.LocalPlayer.PlayerGui

-- Tạo nút bật tắt UI hình tròn
local ToggleButton = Instance.new("ImageButton")
ToggleButton.Parent = Library_Function.HideGui
...
ToggleButton.MouseButton1Click:Connect(function()
	Library_Function.Gui.Enabled = not Library_Function.Gui.Enabled
end)
...
local libCreateNoti = function(Setting)
	cache.TitleNameNoti = Setting.Title or ""; 
	...
	Noticontainer.BackgroundColor3 = Color3.fromRGB(48, 47, 55)
	...
	Ruafimg.Image = cache.UIColor["Logo Image"]

	RuafimgCorner.CornerRadius = UDim.new(1, 0)
	RuafimgCorner.Name = "RuafimgCorner"
	RuafimgCorner.Parent = Ruafimg

	local colorR = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[1])
	local colorG = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[2])
	local colorB = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
	TextLabelNoti.Text = "<font color=\"rgb(" .. color .. ")\">ZMatrix hub</font> "..cache.TitleNameNoti
	...
	TextLabelNoti.Position = UDim2.new(0, cache.T1 and 40 or 35, 0, 0)
	TextLabelNoti.Size = UDim2.new(1, cache.T1 and -40 or -35, 1, 0)
	...
	TextLabelNoti.TextColor3 = cache.UIColor["GUI Text Color"]
	...
	if Desc then
		...
		TextLabelNoti2.TextColor3 = cache.UIColor["Text Color"]
		TextLabelNoti2.AutomaticSize = Enum.AutomaticSize.Y
		TextLabelNoti2.TextWrapped = true
	end

	local function remove()
		TweenService:Create(Noticontainer,TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{Position = UDim2.new(1,0,0,0)}):Play()
		wait(.25)
		NotiFrame:Destroy()
	end

	TweenService:Create(Noticontainer,TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{Position = UDim2.new(0,0,0,0)}):Play()

	TextButton.MouseEnter:Connect(function()
		TweenService:Create(CloseImage, TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{ImageColor3 = cache.UIColor["Search Icon Highlight Color"]}):Play()
	end)

	TextButton.MouseLeave:Connect(function()
		TweenService:Create(CloseImage, TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{ImageColor3 = cache.UIColor["Search Icon Color"]}):Play()
	end)
...
function Library.CreateNoti(Setting, bypass)
	if not cache.Config or bypass then
		local s, e = pcall(function()
			libCreateNoti(Setting)
		end)
		if e then rconsoleprint(e) end
	end
end

function Library.CreateMain(Setting)

	local TitleNameMain =  "by vhai"
	cache.MainDesc = Setting.Desc or ""
	...
	maingui.ImageColor3 = cache.UIColor["Border Color"]
	maingui.ImageTransparency = 1

	maingui.ImageColor3 = cache.UIColor['Title Text Color']
	local colorR = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[1])
	local colorG = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[2])
	local colorB = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
	TextLabelMain.Text = "<font color=\"rgb("..color..")\">Teddy Hub </font> " .. ": Blox Fruits"

	MainContainer = Instance.new("ImageLabel")
	MainContainer.Name = "MainContainer"
	MainContainer.Parent = Main
	MainContainer.BackgroundColor3 = cache.UIColor['Background Main Color']
	MainContainer.Size = UDim2.new(1, 0, 1, 0)
	MainContainer.Transparency = 0.8
	local ustroke = Instance.new("UIStroke", MainContainer)
	ustroke.Thickness = 2
	...
	boder_.Size = UDim2.new(1, 100,1, 157)
	boder_.ZIndex = 0
	...
	TextLabelMain.Position = UDim2.new(0, cache.T1 and 40 or 35, 0, 0)
	TextLabelMain.Size = UDim2.new(1, cache.T1 and -40 or -35, 1, 0)
	...
	TextLabelMain.TextColor3 = cache.UIColor["GUI Text Color"]

	local colorR = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[1])
	local colorG = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[2])
	local colorB = tostring(Library_Function.Getcolor(cache.UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
	TextLabelMain.Text = "<font color=\"rgb("..color..")\">ZMatrix Hub FreeShip </font> " .."- Blox Fruits"

	PageControl.Name = "Background1"
	PageControl.Parent = Concacmain
	PageControl.Position = UDim2.new(0, 5, 0, 0)
	PageControl.Size = UDim2.new(0, 160, 0, 325)
	PageControl.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
	PageControl.BackgroundColor3 = cache.UIColor["Background 1 Color"]
	local thua = Instance.new("UIGradient", PageControl);
	thua["Transparency"] = NumberSequence.new{NumberSequenceKeypoint.new(0.000, 0.1),NumberSequenceKeypoint.new(1.000, 0.1)};
	thua["Name"] = [[thua]];
	thua["Color"] = ColorSequence.new{ColorSequenceKeypoint.new(0.000, Color3.fromRGB(22, 22, 22)),ColorSequenceKeypoint.new(1.000, Color3.fromRGB(44, 44, 44))};
	...
	ControlTitle.TextXAlignment = Enum.TextXAlignment.Center
	ControlTitle.TextColor3 = cache.UIColor["GUI Text Color"]
	...
	MainPage.Position = UDim2.new(0, 170, 0, 0)
	MainPage.Size = UDim2.new(0, 453, 0, 325)
	...
	UIPage.TweenTime = cache.UIColor["Tween Animation 1 Speed"]
	...
	PageSearch.BackgroundColor3 = Color3.fromRGB(37, 36, 44)
	...
	SearchIcon.ImageColor3 = cache.UIColor["Search Icon Color"]
	...
	SearchBox.PlaceholderColor3 = cache.UIColor["Placeholder Text Color"]
	SearchBox.TextColor3 = cache.UIColor["Text Color"]

	local Openned = true

	SearchButton.MouseEnter:Connect(function()
		TweenService:Create(SearchIcon,TweenInfo.new(cache.UIColor["Tween Animation 3 Speed"]),{ImageColor3 = cache.UIColor["Search Icon Highlight Color"]}):Play()
	end)

	SearchButton.MouseLeave:Connect(function()
		TweenService:Create(SearchIcon,TweenInfo.new(cache.UIColor["Tween Animation 3 Speed"]),{ImageColor3 = cache.UIColor["Search Icon Color"]}):Play()
	end)

	SearchButton.MouseButton1Click:Connect(function()
		Openned = not Openned
		local size = Openned and UDim2.new(0,150,0,20) or  UDim2.new(0,20,0,20)
		game.TweenService:Create(PageSearch,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{Size = size}):Play()
	end)
	local size = Openned and UDim2.new(0,150,0,20) or  UDim2.new(0,20,0,20)
	game.TweenService:Create(PageSearch,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{Size = size}):Play()
	local Main_Function = {}
	...
	function Main_Function.CreatePage(Setting)
		...
		InLine.BackgroundColor3 = cache.UIColor["Page Selected Color"]
		...
		TabTitle.TextColor3 = cache.UIColor["GUI Text Color"]
		...
		PageContainer.BackgroundColor3 = cache.UIColor["Background 1 Color"]
		PageContainer.Position = UDim2.new(0, 190, 0, 30)
		PageContainer.Size = UDim2.new(0, 453, 0, 325)
		PageContainer.LayoutOrder = LayoutOrder
		PageContainer.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
		...
		PageTitle.TextColor3 = cache.UIColor["GUI Text Color"]
		...
		SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
			...
			if text == "" then
				for i, v2 in pairs(ControlList:GetChildren()) do
				...
				for i, v in pairs(allfunc) do
					i.Visible = true
					for idx, vl in pairs(v) do
						vl.Visible = true
					end
				end
			else
				...
				for section, functionList in pairs(cache.listfunction) do
					for idx, funcData in pairs(functionList) do
						if string.find(funcData["Name"]:lower(), text:lower()) then
							if funcData["Call"] then
								funcData["Call"]()
							end
							local tabData = cache.listtab[section]
							if tabData then
								tabData[1].Visible = true
								tabData[2].Visible = true
								tabData[3].Visible = true
								local originalTweenTime = cache.UIColor["Tween Animation 1 Speed"]
								UIPage.TweenTime = 0
								UIPage:JumpTo(tabData[2]) 
								task.wait()
								UIPage.TweenTime = originalTweenTime
							end
						end
					end
				end
			...
			for i,v in next, ControlList:GetChildren() do
				if not (v:IsA('UIListLayout')) then
					if v.Name == Page_Name.."_Control" then 
						TweenService:Create(v.Frame.Line.PageInLine,TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{
		...
		function pageFunction.CreateSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			cache.listtab[Section] = {
				[1] = PageName,
				[2] = PageContainer,
				[3] = Section
			}
			cache.listfunction[Section] = {}
			Section.Name = Section_Name.."_Dot"
			Section.Parent = PageList
			Section.Size = UDim2.new(1, -5, 0, 30)
			Section.BackgroundColor3 = cache.UIColor["Background 3 Color"]
			Section.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
			Section.ClipsDescendants = true
			allfunc[Section] = {}
			UICorner.CornerRadius = UDim.new(0, 4)
			UICorner.Parent = Section
			local thua2 = Instance.new("UIGradient",Section);
			thua2["Name"] = [[thua2]];
			thua2["Color"] = ColorSequence.new{ColorSequenceKeypoint.new(0.000, Color3.fromRGB(22, 22, 22)),ColorSequenceKeypoint.new(1.000, Color3.fromRGB(44, 44, 44))};
			...
			Sectiontitle.TextColor3 = SectionColor and SectionColor or  cache.UIColor["Section Text Color"]
			...
			Linesec.BackgroundColor3 = SectionColor and SectionColor or cache.UIColor["Section Underline Color"]
			...
			if Toggleable then
				...
				VisibilityButton.MouseButton1Down:Connect(function()
					...
					TweenService:Create(visibility, TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]/2),{
						ImageTransparency = sectionIsVisible and 0 or 1
					}):Play()
					wait(cache.UIColor["Tween Animation 1 Speed"]/4)
					TweenService:Create(visibility_off, TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]/2),{
						ImageTransparency = sectionIsVisible and 1 or 0
					}):Play()
					TweenService:Create(Section, TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{
			...
			SectionList.Padding = UDim.new(0, 5)
			 ...
			 local lastKnownHeight = -1

			SectionList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                local newHeight = SectionList.AbsoluteContentSize.Y + 5
                
                -- Thêm "điều kiện bảo vệ": Chỉ chạy code nếu chiều cao thực sự thay đổi
                if newHeight == lastKnownHeight then
                    return
                end
                lastKnownHeight = newHeight -- Cập nhật chiều cao mới

				if (not Toggleable) then
					Section.Size = UDim2.new(1, -5, 0, newHeight)
				end
				SizeSectionY = newHeight
			...
			function sectionFunction.CreateToggle(Setting, Callback)
				...
				checkbox.ImageColor3 = cache.UIColor["Toggle Border Color"]
				...
				check.Image = ""
				check.ImageColor3 = cache.UIColor["Toggle Checked Color"]
				...
				if Desc then
					...
					ToggleDesc.TextColor3 = cache.UIColor["Toggle Desc Color"]
				else
					ToggleDesc.Text = ''
				end
				...
				ToggleTitle.TextColor3 = cache.UIColor["Text Color"]

				ToggleBg.Name = "Background1"
				ToggleBg.Parent = TogFrame1
				ToggleBg.Size = UDim2.new(1, 0, 1, 6)
				ToggleBg.ZIndex = 0
				ToggleBg.BackgroundColor3 = cache.UIColor["Background 1 Color"]
				ToggleBg.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
				...
				local thua = Instance.new("UICorner", check)
				thua.CornerRadius = UDim.new(0, 2)
				local function ChangeStage(val)

					local csize = val and UDim2.new(1, -12, 1, -12) or UDim2.new(0,0,0,0)
					local pos = UDim2.new(.5,0,.5,0)
					local apos = Vector2.new(.5,.5)

					game.TweenService:Create(check,TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{Size = csize, Position = pos, AnchorPoint = apos}):Play()
				...
				table.insert(cache.listfunction[Section], {Type = "Toggle",Name = Title, Call = visibleon})
				table.insert(allfunc[Section], ToggleFrame)
...
function sectionFunction.CreateButton(Setting, Callback)
	...
	ButtonBg.BackgroundColor3 = cache.UIColor["Background 1 Color"]
	ButtonBg.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
	...
	ButtonLabel.Parent = ButtonBg
	ButtonLabel.Text = Title
	ButtonLabel.Font = Enum.Font.GothamBold
	ButtonLabel.TextSize = 14
	ButtonLabel.TextColor3 = cache.UIColor["Text Color"]
	ButtonLabel.BackgroundTransparency = 1
	ButtonLabel.Position = UDim2.new(0, 12, 0, 0)
	ButtonLabel.Size = UDim2.new(1, -110, 1, 0)
	ButtonLabel.TextXAlignment = Enum.TextXAlignment.Left
	ButtonLabel.ClipsDescendants = false

	-- Click Button Area - GÓC BO TRÒN GẦN NHƯ HÌNH TRÒN
	local ClickButton = Instance.new("TextButton")
	ClickButton.Name = "ClickButton"
	ClickButton.Parent = ButtonBg
	ClickButton.AnchorPoint = Vector2.new(1, 0.5)
	ClickButton.Size = UDim2.new(0, 94, 0, 30)
	ClickButton.Position = UDim2.new(1, -8, 0.5, 0)
	...
	ClickButtonCorner.CornerRadius = UDim.new(1, 0) -- GÓC BO TRÒN TỐI ĐA (như trong ảnh)
	...
	-- Hiệu ứng phản chiếu (như trong ảnh)
	local ButtonReflection = Instance.new("Frame")
	ButtonReflection.Name = "ButtonReflection"
	ButtonReflection.Parent = ClickButton
	ButtonReflection.AnchorPoint = Vector2.new(0.5, 0)
	ButtonReflection.BackgroundColor3 = Color3.new(1, 1, 1)
	ButtonReflection.BackgroundTransparency = 0.85
	ButtonReflection.Size = UDim2.new(1, -6, 0, 10)
	ButtonReflection.Position = UDim2.new(0.5, 0, 0, 2)
	ButtonReflection.ZIndex = 2

	local ReflectionCorner = Instance.new("UICorner")
	ReflectionCorner.CornerRadius = UDim.new(0, 4)
	ReflectionCorner.Parent = ButtonReflection

	-- Hiệu ứng click (scale animation) - GIỐNG TRONG ẢNH
	local isAnimating = false
	ClickButton.MouseButton1Click:Connect(function()
		if not isAnimating then
			isAnimating = true
			
			-- Lưu màu gốc và kích thước gốc
			local originalColor = ClickButton.BackgroundColor3
			local originalSize = ClickButton.Size
			
			-- Hiệu ứng nhấn xuống
			game:GetService("TweenService"):Create(ClickButton, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				Size = UDim2.new(0, 90, 0, 28),
				Position = UDim2.new(1, -8, 0.5, 1),
				BackgroundColor3 = Color3.fromRGB(165, 239, 35) -- Sáng hơn khi nhấn
			}):Play()
			
			wait(0.08)
			
			-- Hiệu ứng nhả ra
			game:GetService("TweenService"):Create(ClickButton, TweenInfo.new(0.12, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {
				Size = originalSize,
	...
	-- Function để hiển thị button
	function visibleon()
		ButtonContainer.Visible = true
	end

	-- Thêm vào cache
	table.insert(cache.listfunction[Section], {Type = "Button", Name = Title, Call = visibleon})
	table.insert(allfunc[Section], ButtonContainer)
	...
	return {}
			...
			function sectionFunction.CreateLabel(Setting)
				...
				LabelBG.BackgroundColor3 = cache.UIColor["Background 1 Color"]
				LabelBG.AutomaticSize = Enum.AutomaticSize.Y
				LabelBG.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
				...
				LabelTitle.Size = UDim2.new(1, -20, 1, 10)
				...
				LabelTitle.TextColor3 = cache.UIColor["Text Color"]
				LabelTitle.TextStrokeTransparency = cache.UIColor["Text Stroke Transparency"]
				...
				table.insert(cache.listfunction[Section], {Type = "Label",Name = Title, Call = visibleon})
				table.insert(allfunc[Section], LabelFrame)
			...
			function sectionFunction.CreateDropdown(Setting, Callback)
				...
				Dropdownbg.BackgroundColor3 = Color3.fromRGB(28, 28, 34)
				Dropdownbg.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
				...
				Topdrop.BackgroundColor3 = Color3.fromRGB(38, 38, 46)
				Topdrop.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
				Topdrop.BackgroundTransparency = 0.25
				UICorner.CornerRadius = UDim.new(0, 4)
				...
				if not Selected then
					if Search then
						Dropdowntitle.PlaceholderColor3 = cache.UIColor["Placeholder Text Color"]
				...
				Dropdowntitle.TextColor3 = cache.UIColor["Text Color"]
				...
				ImgDrop.ImageColor3 = cache.UIColor["Dropdown Icon Color"]
				...
				local function refreshlist(SortPairs)
					...
					if Selected then 
						...
						for i,v in pairs (ListNew) do
							...
							SampleItemBG.BackgroundColor3 = v and cache.UIColor["Dropdown Selected Check Color"] or Color3.fromRGB(255, 255, 255)
							...
							SampleItemCheck.ImageColor3 = cache.UIColor["Dropdown Selected Check Color"]
							...
							SampleItemButton.TextColor3 = cache.UIColor["Text Color"]
							...
							SampleItemButton.MouseEnter:Connect(function()
								...
								TweenService:Create(
									SampleItemBG,
									TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{
							...
							SampleItemButton.MouseButton1Click:Connect(function()
								...
								TweenService:Create(
									SampleItemBG,
									TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{
										BackgroundColor3 = v and cache.UIColor["Dropdown Selected Check Color"] or Color3.fromRGB(255, 255, 255)
									}
								):Play()
					...
					elseif Slider then
						for i,v in pairs(ListNew) do
							...
							SliderBG.BackgroundColor3 = cache.UIColor["Background 1 Color"]
							SliderBG.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
							...
							SliderTitle.TextColor3 = cache.UIColor["Text Color"]
							...
							SliderBar.BackgroundColor3 = cache.UIColor["Background 2 Color"]
							...
							Bar.BackgroundColor3 = cache.UIColor["Slider Line Color"]
							...
							Sliderboxframe.BackgroundColor3 = cache.UIColor["Background 2 Color"]
							...
							Sliderbox_2.TextColor3 = cache.UIColor["Text Color"]

							SliderButton.MouseEnter:Connect(function()
								TweenService:Create(Bar,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{BackgroundColor3 = cache.UIColor["Slider Highlight Color"]}):Play()
							end)

							SliderButton.MouseLeave:Connect(function()
								TweenService:Create(Bar,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{BackgroundColor3 = cache.UIColor["Slider Line Color"]}):Play()
							end)
							...
							if typeof(v) == "string" then
								...
								SampleItemButton.MouseButton1Click:Connect(function()
									...
									TweenService:Create(
										SampleItemBG,
										TweenInfo.new(cache.UIColor["Tween Animation 1 Speed"]),{
											BackgroundColor3 = cache.UIColor["Dropdown Selected Check Color"]
										}
									):Play()
								...
								if Sel.Value == v then
									...
									SampleItemBG.BackgroundColor3 = cache.UIColor["Dropdown Selected Check Color"]
				...
				DropdownButton.MouseButton1Click:Connect(function()
					...
					TweenService:Create(Dropdownlisttt,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{Size = listsize}):Play()
					TweenService:Create(DropdownFrame,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{Size = mainsize}):Play()
					TweenService:Create(ImgDrop,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{Rotation = DropCRotation}):Play()
				...
				table.insert(cache.listfunction[Section], {Type = "Dropdown",Name = Title,Call = function()
					DropdownFrame.Visible = true
				end,})
				table.insert(allfunc[Section], DropdownFrame)
			...
			function sectionFunction.CreateBind(Setting, Callback)
				...
				table.insert(cache.listfunction[Section], {Type = "Bind",Name = TitleText})
				...
				BindBG.BackgroundColor3 = cache.UIColor["Background 1 Color"]
				BindBG.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
				...
				BindButtonTitle.TextColor3 = cache.UIColor["Text Color"]
				...
				BindCor.BackgroundColor3 = cache.UIColor["Background 2 Color"]
				...
				Bindkey.TextColor3 = cache.UIColor["Text Color"]
			...
			function sectionFunction.CreateBox(Setting, Callback)
				...
				BoxBG.BackgroundColor3 = cache.UIColor["Background 1 Color"]
				BoxBG.BackgroundTransparency = cache.UIColor["Background 1 Transparency"]
				...
				Boxtitle.TextColor3 = cache.UIColor["Text Color"]
				...
				BoxCor.BackgroundColor3 = cache.UIColor["Background 2 Color"]
				...
				Boxxx.PlaceholderColor3 = cache.UIColor["Placeholder Text Color"]
				Boxxx.TextColor3 = cache.UIColor["Text Color"]
				...
				Lineeeee.BackgroundColor3 = cache.UIColor["Box Highlight Color"]
				...
				Boxxx.Focused:Connect(function() 
					TweenService:Create(Lineeeee,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{BackgroundTransparency = 0}):Play()
				end)
				...
				Boxxx.FocusLost:Connect(function()
					TweenService:Create(Lineeeee,TweenInfo.new(cache.UIColor["Tween Animation 2 Speed"]),{BackgroundTransparency = 1}):Play()
					if Boxxx.Text ~= '' then
						Callback(Boxxx.Text)
					end
				end)
				...
				table.insert(cache.listfunction[Section], {Type = "Box",Name = TitleText, Call=function()
					BoxFrame.Visible = true
				end,})
				table.insert(allfunc[Section], BoxFrame)
			...
			function sectionFunction.CreateSlider(Setting, Callback)
				...
				table.insert(cache.listfunction[Section], {Type = "Slider",Name = TitleText, Call = function()
					SliderFrame.Visible = true
				end,})
				table.insert(allfunc[Section], SliderFrame)
...
Library.CreateNoti({Title = 'Notification', Desc = 'The UI automatically hides once executed.\nPress the button at the bottom-left of the screen to show the GUI.', ShowTime = 10}, true)
return Library